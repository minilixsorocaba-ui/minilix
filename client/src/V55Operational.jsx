...
export default function V55Operational(){
 ...
 const pickupOrders=orders.filter(o=>o.order_type==='RETIRADA');
 return <>
  ...
  {open&&<div className="v55-overlay" onClick={e=>{if(e.target===e.currentTarget)setOpen(false)}}>
   <section className="v55-panel">
    ...
    <div className="v55-section"><h3>Retiradas</h3>
      {pickupOrders.slice(0,10).map(o=><div className="v55-row" key={`r-${o.id}`}><div><b>{o.number}</b><span>{o.customer_name} · retirada {labels[o.status].toLowerCase()}</span><small>{o.scheduled_date} · {o.driver_name||'Sem motorista'}</small></div><div className="actions">{o.route_url&&<a className="route" href={o.route_url} target="_blank" rel="noreferrer">🗺️ Rota</a>}<span className="badge">{labels[o.status]}</span></div></div>)}
      {pickupOrders.length===0&&<div className="empty">Nenhuma OS de retirada criada.</div>}
      <p className="muted" style={{marginTop:12}}>A OS de retirada é criada automaticamente quando a entrega é concluída. Ao concluir a retirada, o sistema encerra a locação e devolve os tambores ao estoque.</p>
    </div>
