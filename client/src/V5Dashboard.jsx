...
export default function V5Dashboard(){
  ...
  const load=async()=>{setLoading(true);setErr('');try{const [s,o,a,f]=await Promise.all([api('/api/dashboard'),api('/api/orders'),api('/api/assets'),api('/api/finance')]);setData({s,o,a,f})}catch(e){setErr(e.message)}finally{setLoading(false)}};
  ...
  return <>
    ...
    {open&&<div style={{position:'fixed',right:22,bottom:78,zIndex:9998,width:'min(430px,calc(100vw - 32px))',maxHeight:'calc(100vh - 110px)',overflow:'auto',background:'#fff',borderRadius:18,boxShadow:'0 18px 55px rgba(0,0,0,.22)',padding:20,border:'1px solid #e5e7eb'}}>
      ...
      {data&&<>
        ...
        <div style={{marginTop:16}}><h3 style={{marginBottom:8}}>Financeiro em aberto</h3><p style={{margin:0}}><b>{pendingFinance.length}</b> lançamento(s) em aberto.</p>{pendingFinance.slice(0,5).map(x=><div key={x.id} style={{padding:'8px 0',fontSize:13}}>{x.description||x.type} · <b>{money(x.amount)}</b></div>)}</div>
        <div style={{marginTop:16,padding:12,borderRadius:12,background:'#f8fafc',fontSize:13}}>Esta é a primeira camada da V5: visão gerencial, agenda diária, alertas operacionais e leitura financeira em um só lugar.</div>
      </>}
    </div>}
  </>;
}
