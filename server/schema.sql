...
CREATE TABLE IF NOT EXISTS addresses (
  ...
  cep TEXT, street TEXT NOT NULL, number TEXT NOT NULL, complement TEXT,
  neighborhood TEXT, city TEXT NOT NULL DEFAULT 'Sorocaba', state TEXT NOT NULL DEFAULT 'SP', reference TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE TABLE IF NOT EXISTS containers (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  name TEXT NOT NULL, capacity_liters INTEGER NOT NULL, active BOOLEAN NOT NULL DEFAULT TRUE,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS container_assets (
  ...
  status TEXT NOT NULL DEFAULT 'DISPONIVEL' CHECK (status IN ('DISPONIVEL','ALUGADO','MANUTENCAO','INATIVO')),
  notes TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS rental_items (
  ...
  container_id UUID NOT NULL REFERENCES containers(id),
  asset_id UUID REFERENCES container_assets(id),
...
CREATE TABLE IF NOT EXISTS service_orders (
  ...
  order_type TEXT NOT NULL DEFAULT 'ENTREGA' CHECK (order_type IN ('ENTREGA','RETIRADA')),
  status TEXT NOT NULL DEFAULT 'PENDENTE' CHECK (status IN ('PENDENTE','ATRIBUIDA','A_CAMINHO','NO_LOCAL','CONCLUIDA','CANCELADA')),
  scheduled_date DATE NOT NULL, started_at TIMESTAMPTZ, completed_at TIMESTAMPTZ,
  driver_notes TEXT, route_url TEXT, photo_data TEXT, customer_confirmation TEXT, confirmed_at TIMESTAMPTZ,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_assets_status ON container_assets(status);
CREATE INDEX IF NOT EXISTS idx_events_rental ON rental_events(rental_id,created_at DESC);
CREATE INDEX IF NOT EXISTS idx_financial_status ON financial_entries(status,due_date);
...
-- Migração idempotente para instalações já existentes.
...
BEGIN
  ...
  IF OLD.status = 'PENDENTE' AND NEW.status NOT IN ('ATRIBUIDA','CANCELADA') THEN RAISE EXCEPTION 'Transição de OS inválida: PENDENTE -> %',NEW.status;
  ELSIF OLD.status = 'ATRIBUIDA' AND NEW.status NOT IN ('PENDENTE','A_CAMINHO','CANCELADA') THEN RAISE EXCEPTION 'Transição de OS inválida: ATRIBUIDA -> %',NEW.status;
  ELSIF OLD.status = 'A_CAMINHO' AND NEW.status NOT IN ('NO_LOCAL','CANCELADA') THEN RAISE EXCEPTION 'Transição de OS inválida: A_CAMINHO -> %',NEW.status;
  ELSIF OLD.status = 'NO_LOCAL' AND NEW.status NOT IN ('CONCLUIDA','CANCELADA') THEN RAISE EXCEPTION 'Transição de OS inválida: NO_LOCAL -> %',NEW.status;
  ELSIF OLD.status IN ('CONCLUIDA','CANCELADA') THEN RAISE EXCEPTION 'OS finalizada não pode voltar de status: %',OLD.status;
  END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_service_order_status_transition ON service_orders;
CREATE TRIGGER trg_service_order_status_transition BEFORE UPDATE OF status ON service_orders FOR EACH ROW EXECUTE FUNCTION enforce_service_order_status_transition();

-- Gera ativos patrimoniais para o catálogo inicial quando ainda não existem.
INSERT INTO container_assets(container_id,patrimony_code)
SELECT c.id,'ML-'||c.capacity_liters||'-'||LPAD(ROW_NUMBER() OVER (PARTITION BY c.capacity_liters ORDER BY c.id)::text,3,'0')
FROM containers c
WHERE NOT EXISTS (SELECT 1 FROM container_assets a WHERE a.container_id=c.id)
LIMIT 3;
